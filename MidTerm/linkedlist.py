...
class LinkedList:
    ...
    def insert(self, cursor: LinkedNode, new_value: Any) -> None:
        ...
        if cursor == self.front:
            ...
            node = self.front
            while node.get_link() != cursor:
                node = node.get_link()
            node.set_link(LinkedNode(new_value, cursor))
    ...
    def _size_to_end(self, node: LinkedNode) -> int:
        if node is None:
            ...
            return 1 + self._size_to_end(node.get_link())
